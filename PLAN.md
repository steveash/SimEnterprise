...
| D26 | **Engine = deterministic event-driven discrete-event simulation.** Stable tie-breaking + split-RNG sub-streams; **scheduling is deterministic-sequential, only rendering parallelizes**; `OnCondition` via effect-driven re-evaluation (+ coarse safety tick). | Concurrency never perturbs *what* happens, only render speed. See `ARCHITECTURE.md` §15. |
| D27 | **Step placement = greedy soft-constraint** against a per-person busy map (overlap only if forced, and logged); per-person calendars derive from the busy map. **Business-hour granularity** in v1 (weekday 9–17 local), tz-aware later. | Believable calendars without a constraint solver. |
| D28 | **Actor/relationship resolver = affinity + capacity.** Rank candidates by affinity + inverse-load + expertise; picking **reinforces affinity** (preferential attachment with capacity caps); resolver writes relationship edges. Latent affinities seeded in Layer A. | Go-to experts and collaborator clusters self-organize → realistic collaboration + richer KG. |
| D29 | **Layered, cache-aware prompt assembly:** system → company → scenario/project (cacheable prefix, `cache_control` breakpoints) → task brief + roster (volatile). **Layer C renders clustered by shared prefix** to keep the cache warm. Prompt templates owned by plugins. | Cuts cost on api/bedrock; cli backend skips caching. See `ARCHITECTURE.md` §16. |

...
_SYSTEM_PROMPT = (
    ...
    "objective — a bold bullet under a Goals heading, or a line under an "
    "Advances-goals list — emit it as a Goal mention whose surface form is the "
    "COMPLETE statement sentence, copied verbatim (keep trailing punctuation; drop "
    'only surrounding markdown like "**"). Then, when goals nest, emit subgoal_of '
    "from the indented sub-goal to its parent goal, and advances_goal from a "
    "department or initiative to the goal it advances — naming each goal endpoint by "
    "that same full statement.\n"
